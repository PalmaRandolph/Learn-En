...
class SettingController: UIViewController {
    ...
    fileprivate var titles = ["0": "单词搜索:更多单词搜索,语言设置:设置英文单词翻译成的语言",
                              "1": "分享给朋友:与身边的好友一起分享！",
        "2":"问题反馈:欢迎提交问题,邮件联系:如有意见欢迎来信,开源地址:开放代码，欢迎关注,关于应用:Learn-\(kAppName)"] as [String : String]
...
//                gotoAppstore(isAssessment: true)
                if #available(iOS 9.0, *) {
                    let sfvc = SFSafariViewController(url: URL(string: kGithubURLissues
                    ...
                    if UIApplication.shared.canOpenURL(URL.init(string: kGithubURLissues )!) {
                        UIApplication.shared.openURL(URL.init(string: kGithubURLissues)!)
                    }
                }
            }
            if row == 1 {
                let message = "欢迎来信，写下你的意见吧" + "\n\n\n\n" + kMarginLine + "\n 当前\(kAppName)版本：" + KAppVersion + "， 系统版本：" + String(Version.SYS_VERSION_FLOAT) + "， 设备信息：" + UIDevice.init().modelName
